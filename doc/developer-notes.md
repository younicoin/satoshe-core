...
- [Development guidelines](#development-guidelines)
    - [General SATOSHE](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SATOSHE uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
SATOSHE is a multi-threaded application, and deadlocks or other
...
SATOSHE can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as SATOSHE, where everyone uses
...
pay attention to for reviewers of SATOSHE code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: SATOSHE uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of SATOSHE, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against SATOSHE so that they can be integrated
...
file descriptors, it will cause problems with SATOSHE's `select()` loop, because
...
compatibility between nodes. This happened in SATOSHE 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to SATOSHE's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of SATOSHE, the wallet was always
